...
int testPutInexistente() {
	...
	//printf("tree_size -> %i\n", tree_size(tree));
	//printf("tree-datasize -> %i\n", tree->entry->value->datasize);
	//printf("Primeira key -> %s\n", tree->right->right->entry->key);
	result = (tree_size(tree) == 1024);

	int counter = 1;

	for(i=0; i<1024; i++) {
		printf("Antes tree_get\n");
		printf("Ciclo %i\n", counter);
		d = tree_get(tree,key[i]);
		printf("Depois do tree_get\n");
